...
class ReactQuillES6 extends Component {
    ...
    renderContents() {
        if (React.Children.count(this.props.children)) {
            return React.Children.map(this.props.children, (c) => {
                return React.cloneElement(c, {ref: c.ref})
            })
        } else {
            return [
                this.props.toolbar !== false
                    ? <QuillToolbar key={`toolbar-${Math.random()}`} ref='toolbar' items={this.props.toolbar}/>
                    : null, < div key = {
                    `editor-${Math.random()}`
                }
                ref = 'editor' className = 'quill-contents' dangerouslySetInnerHTML = {{ __html: this.getEditorContents() }} > </div>
            ]
        }
    }
    ...
    render() {
        return (
            <div id={this.props.id} style={this.props.style} className={['quill'].concat(this.props.className).join(' ')} onKeyPress={this.props.onKeyPress} onKeyDown={this.props.onKeyDown} onKeyUp={this.props.onKeyUp} onChange={this.preventDefault}>
                {this.renderContents()}
            </div>
        );
    }
